Ejercicio 1: Contar líneas de un archivo
Escribe un script que solicite el nombre de un archivo y luego imprima cuántas líneas tiene ese archivo. Verifica que el archivo exista antes de contar las líneas.

Creo el fichero para el script:
```bash
cesar@crb-server:~$ touch script1.sh
cesar@crb-server:~$ ls
pr0201  script1.sh
```

Compruebo el contenido de mi fichero conejillo de indias:
```bash
cesar@crb-server:~$ cat pr0201/file
bliblablu
blublabli
cesar@crb-server:~$
```

Añado al script1.sh:

```bash
#!/bin/bash
echo "Ingrese el nombre del archivo con su ruta:"
read archivo

if [ -f $archivo ]; then
        echo "El archivo tiene el siguiente número de líneas:"
        wc -l $archivo
else
        echo "El archivo no existe"
fi
```

Compruebo que funciona:

```bash
root@crb-server:/home/cesar# ./script1.sh
Ingrese el nombre del archivo con su ruta:
pr0201/file
El archivo tiene el siguiente número de líneas:
2 pr0201/file
root@crb-server:/home/cesar#
```

Ejercicio 2: Verificación de archivo
Crea un script que compruebe si un archivo (cuya ruta pedirá al usuario por teclado) existe y si tiene permisos de lectura. Muestra un mensaje adecuado para cada caso.

Hago el script:

```bash
#!/bin/bash
echo "Ingrese el nombre del archivo con su ruta:"
read archivo

if [ -r $archivo ]; then
        echo "El archivo tiene permisos de lectura"
else
        echo "El archivo no tiene permisos de lectura"
fi
```

Comprobamos el funcionamiento como usuario:

```bash
cesar@crb-server:~$ ./script2.sh
Ingrese el nombre del archivo con su ruta:
pr0201/file
El archivo tiene permisos de lectura
cesar@crb-server:~$ ./script2.sh
Ingrese el nombre del archivo con su ruta:
/etc/shadow
El archivo no tiene permisos de lectura
cesar@crb-server:~$
```

Ejercicio 3: Comparación de dos números
Realiza un script que solicite dos números al usuario y los compare, mostrando cuál es mayor, o si son iguales.

Para solicitar al usuario que tiene que introducir un valor por teclado tienes que utilizar el comando read tal como se muestra en el siguiente ejemplo:

read -p "Introduce tu usuario: " usuario
echo Bienvenido, $usuario

Hacemos el script:

```bash
#!/bin/bash
echo "Ingrese el primer número:"
read primernum
echo "Ingrese el segundo número:"
read segundonum

if [ $primernum -eq $segundonum ]; then
        echo "$primernum es igual a $segundonum"
else

        if [ $primernum -gt $segundonum ]; then
                echo "$primernum es mayor que $segundonum"
        else
                echo "$primernum es menor que $segundonum"
        fi
fi
```

Comprobamos el funcionamiento:

```bash
cesar@crb-server:~$ ./script3.sh
Ingrese el primer número:
2
Ingrese el segundo número:
2
2 es igual a 2
cesar@crb-server:~$ ./script3.sh
Ingrese el primer número:
5
Ingrese el segundo número:
2
5 es mayor que 2
cesar@crb-server:~$ ./script3.sh
Ingrese el primer número:
2
Ingrese el segundo número:
5
2 es menor que 5
cesar@crb-server:~$
```

Ejercicio 4: Validación de contraseña
Escribe un script que solicite al usuario una contraseña y verifique si coincide con una contraseña predefinida (que estará almacenada en una variable de tu script). Si es correcta, muestra un mensaje de éxito, de lo contrario, indica que es incorrecta.

Hacemos el script:

```bash
#!/bin/bash
contrareal=1234
echo "Ingrese la contraseña:"
read contradada

if [ $contradada = $contrareal ]; then
        echo "¡Bienvenido!"
else
        echo "Contraseña incorrecta"
fi
```

Comprobamos el funcionamiento:

```bash
cesar@crb-server:~$ ./script4.sh
Ingrese la contraseña:
blabla
Contraseña incorrecta
cesar@crb-server:~$ ./script4.sh
Ingrese la contraseña:
1234
¡Bienvenido!
cesar@crb-server:~$
```

Ejercicio 5: Comprobación de directorio
Crea un script que compruebe si un directorio (que solicitará por teclado al usuario) existe y si tiene permisos de escritura. Si el directorio no existe, crea uno nuevo.

Hacemos el script:

```bash
#!/bin/bash
echo "Ingrese el nombre del directorio:"
read direc

if [ -d $direc ] && [ -w $direc ]; then
        echo "El directorio existe y tiene permisos de escritura."
else
        echo "El directorio no existía. Procedemos a crearlo con el nombre proporcionado:"
        mkdir $direc
fi
```

Comprobamos el funcionamiento:

```bash
cesar@crb-server:~$ ./script5.sh
Ingrese el nombre del directorio:
pr0201
El directorio existe y tiene permisos de escritura.
cesar@crb-server:~$ ./script5.sh
Ingrese el nombre del directorio:
blublabli
El directorio no existía. Procedemos a crearlo con el nombre proporcionado:
cesar@crb-server:~$ ls
blublabli  pr0201  script1.sh  script2.sh  script3.sh  script4.sh  script5.sh
cesar@crb-server:~$
```

Ejercicio 6: Verificar si el usuario es root
Haz un script que verifique si el script está siendo ejecutado por el usuario root, mostrando un mensaje diferente si no lo es.

Hacemos el script:

```bash
#!/bin/bash
if [ $USER = root ]; then
        echo "Eres el root"
else
        echo "No eres el root"
fi
```

Comprobamos su funcionamiento:

```bash
cesar@crb-server:~$ ./script6.sh
No eres el root
cesar@crb-server:~$ sudo su
[sudo] password for cesar:
root@crb-server:/home/cesar# ./script6.sh
Eres el root
root@crb-server:/home/cesar#
```

Ejercicio 7: Calificación de un examen
Realiza un script que pida una nota numérica y determine si es “Aprobado” (5 o más) o “Suspenso” (menos de 5).

Hacemos el script:

```bash
#!/bin/bash

echo "¿Cuál es tu nota?"
read nota

if [ $nota -ge 5 ]; then
        echo "Aprobado"
else
        echo "Suspenso"
fi
```

Comprobamos su funcionamiento:

```bash
cesar@crb-server:~$ ./script7.sh
¿Cuál es tu nota?
5
Aprobado
cesar@crb-server:~$ ./script7.sh
¿Cuál es tu nota?
4
Suspenso
cesar@crb-server:~$ ./script7.sh
¿Cuál es tu nota?
7
Aprobado
cesar@crb-server:~$
```

Ejercicio 8: Comprobación del espacio en disco
Crea un script que compruebe el espacio libre en disco. Si el espacio es inferior al 10%, muestra un mensaje de advertencia.

Hacemos el script:

```bash
#!/bin/bash

uso=$(df / | tail -1 | awk '{print $5}' | sed 's/%//')

libre=$((100 - uso))

if [ "$libre" -lt 10 ]; then
    echo "Advertencia: El espacio libre en disco es menor al 10% (solo queda ${libre}%)."
else
    echo "Espacio suficiente en disco: ${libre}% libre."
fi
```

Comprobamos su funcionamiento:

```bash
cesar@crb-server:~$ ./script8.sh
Espacio suficiente en disco: 73% libre.
```

Ejercicio 9: Menú de opciones
Escribe un script que muestre un menú con tres opciones. El usuario debe introducir una opción y el script debe ejecutar una acción diferente dependiendo de la opción seleccionada (es suficiente con que muestre un mensaje diferente según la opción escogida)

Hacemos el script:

```bash
#!/bin/bash

echo "Elige una opción entre estas tres:"
echo "a - Opción A"
echo "b - Opción B"
echo "c - Opción c"
read opc

if [ $opc = 'a' ]; then
        echo "Has elegido la opción A"
elif [ $opc = 'b' ]; then
        echo "Has elegido la opción B"
elif [ $opc = 'c' ]; then
        echo "Has elegido la opción C"
else
        echo "Error de input"
fi
```

Comprobamos el funcionamiento:

```bash
cesar@crb-server:~$ ./script9.sh
Elige una opción entre estas tres:
a - Opción A
b - Opción B
c - Opción c
a
Has elegido la opción A
cesar@crb-server:~$ ./script9.sh
Elige una opción entre estas tres:
a - Opción A
b - Opción B
c - Opción c
b
Has elegido la opción B
cesar@crb-server:~$ ./script9.sh
Elige una opción entre estas tres:
a - Opción A
b - Opción B
c - Opción c
c
Has elegido la opción C
cesar@crb-server:~$
```

Ejercicio 10: Evaluación de edad
Haz un script que solicite al usuario su edad y determine si es menor, adulto o mayor de edad, según un umbral predefinido (por ejemplo, menor de 18, entre 18 y 65, y mayor de 65).

Hacemos el script:

```bash
#!/bin/bash

echo "Introduce tu edad:"
read edad

if [ $edad -lt 18 ]; then
        echo "Eres menor"
elif [ $edad -ge 18 ] && [ $edad -le 65 ]; then
        echo "Eres adulto"
elif [ $edad -gt 65 ]; then
        echo "Eres anciano"
else
        echo "Error de input"
fi
```

Comprobamos el funcionamiento:

```bash
cesar@crb-server:~$ ./script10.sh
Introduce tu edad:
18
Eres adulto
cesar@crb-server:~$ ./script10.sh
Introduce tu edad:
17
Eres menor
cesar@crb-server:~$ ./script10.sh
Introduce tu edad:
50
Eres adulto
cesar@crb-server:~$ ./script10.sh
Introduce tu edad:
65
Eres adulto
cesar@crb-server:~$ ./script10.sh
Introduce tu edad:
66
Eres anciano
cesar@crb-server:~$
```