...
Ahora vamos a crear nuestro script en NETLOGON (Se entra con \\\localhost\NETLOGON) para hacer que la carpeta se monte automáticamente como unidad de red en los equipos de los usuarios:

![Ejemplo15](./15.png)

(Configuré el archivo conecta.bat primero como txt y le escribí la línea net use h: \\\crbcontrolador\shares\apuntes)

Ahora se especifica lo siguiente para los usuarios del grupo en propiedades - perfil:

![Ejemplo16](./16.png)

Y comprobamos con la cuenta de alu_asir_1 en el W10cliente:

(Resulta que haber escrito net use h: \\\crbcontrolador\shares\apuntes genera conflicto con lo realizado anteriormente en esta práctica, debemos usar un letra distinta para el montaje. Vamos a usar la I:)

Una vez cambiado a net use I: \\\crbcontrolador\shares\apuntes comprobamos:

![Ejemplo17](./17.png)

Usando el comando net use también vemos:
...
El proceso es el mismo, hacemos la carpeta y la compartimos al grupo con los permisos; y necesitamos otra letra por lo que añadimos al conecta.bat lo siguiente:

![Ejemplo19](./19.png)

Ahora comprobamos desde alu_asir_2 en W10cliente:

![Ejemplo20](./20.png)

Esta captura demuestra que los permisos de lectura y escritura funcionan y está compartido correctamente mediente el script.