...
Igual que antes, lo creamos y vinculamos:
...
Tras ir allí (bastante profundo) ya lo tenemos:
...
Comprobamos desde el W10cliente (entré en alu_asir_1 que es el que tenía creado):

![Ejemplo15](./15.png)

No tiene la unidad Z:.

Inicia sesión con un alumno de administración (ej. alu_afi_1). Verifica que SÍ aparece la unidad Z:.

Comprobamos desde el W10cliente:

(Hubo que hacer el proceso visto ya en ocasiones anteriores de crear la carpeta y compartirla en el crbcontrolador dándole permisos al grupo GRP_Alumnos_AFI que contiene a los alumnos de AFI para que se mostrara en el W10cliente. Ojo que le di permisos a compartido y a administracion por igual por si acaso. NO HIZO FALTA CREAR UN RECURSO COMPARTIDO DESDE EL ADMINISTRADOR DEL SERVIDOR)

![Ejemplo16](./16.png)

Tiene la unidad Z: y accede bien a la carpeta administracion.