Bloque I: Arrays fijos

1. Reconfiguración de DNS y Verificación

Como administrador, tienes un script que carga los servidores DNS de una sucursal en un array fijo. Se ha detectado que el servidor secundario está caído y la configuración actual es errónea.

Pasos a realizar:

Declara el array con los valores iniciales: 192.168.1.10 (Primario) y 10.0.0.50 (Secundario erróneo).

```bash
PS C:\Users\Alumno> $servidores = @("192.168.1.10","10.0.0.50")
```

Muestra en pantalla: “Configuración actual: [Primario] - [Secundario]”.

```bash
PS C:\Users\Alumno> Write-Host "Configuración actual: [$($servidores[0])] - [$($servidores[1])]"
Configuración actual: [192.168.1.10] - [10.0.0.50]
```

Accede directamente al índice correspondiente y cambia la IP del secundario por la de Google: 8.8.8.8.

```bash
PS C:\Users\Alumno> $servidores[1] = "8.8.8.8"
PS C:\Users\Alumno> Write-Host "$($servidores[1])"
8.8.8.8
```

Muestra en pantalla el número total de servidores DNS configurados.

```bash
PS C:\Users\Alumno> $total = $servidores.Count
PS C:\Users\Alumno> $total
2
```

Muestra la configuración final corregida.

```bash
PS C:\Users\Alumno> Write-Host "Configuración actual: [$($servidores[0])] - [$($servidores[1])]"
Configuración actual: [192.168.1.10] - [8.8.8.8]
```

2. Rotación de logs de backups (LIFO - Last In, First Out)

Un sistema antiguo guarda los nombres de los últimos 3 backups en un array. Debes identificar cuál es el más antiguo y cuál el más nuevo para un informe, asumiendo que el índice 0 es el más antiguo.

Pasos a realizar:

Declara el array con las siguientes cadenas: Backup_Lunes.zip, Backup_Martes.zip, Backup_Miercoles.zip.

```bash
PS C:\Users\Alumno> $backups = @("Backup_Lunes.zip","Backup_Martes.zip","Backup_Miercoles.zip")
```

Guarda en una variable $oldest el primer elemento.

```bash
PS C:\Users\Alumno> $oldest = $backups[0]
```

Guarda en una variable $newest el último elemento.

```bash
PS C:\Users\Alumno> $newest = $backups[2]
```

Otra forma:

```bash
PS C:\Users\Alumno> $newest = $backups[-1]
```

El backup del Miércoles ha salido corrupto. Modifica el último elemento del array añadiéndole el texto “ (CORRUPTO)” al final del nombre.

```bash
PS C:\Users\Alumno> $backups[2] = "Backup_Miercoles.zip(CORRUPTO)"
```

Imprime un resumen: “Rotación de backups: Del [Viejo] al [Nuevo]”.

```bash
PS C:\Users\Alumno> Write-Host "Rotación de backups: Del [$($backups[0])] al [$($backups[2])]"
Rotación de backups: Del [Backup_Lunes.zip] al [Backup_Miercoles.zip(CORRUPTO)]
```

Bloque II: ArrayLists

3. Gestión de Cola de incidencias (Priorización)

Estás programando un dashboard de soporte técnico. Las incidencias llegan y se ponen en cola. De repente, llega una incidencia crítica del CEO que debe saltarse la cola.

Pasos a realizar:

Crea un ArrayList vacío.

```bash
PS C:\Users\Alumno> $incidencia = New-Object System.Collections.ArrayList
```

Añade las incidencias: “Monitor parpadea” y “Ratón no va”. (Recuerda ocultar la salida por pantalla del .Add()).

```bash
PS C:\Users\Alumno> [void]$incidencia.Add("Monitor parpadea")
PS C:\Users\Alumno> [void]$incidencia.Add("Ratón no va")
PS C:\Users\Alumno> $incidencia
Monitor parpadea
Ratón no va
```

Llega una urgencia: Inserta en la posición 0 la incidencia: “SERVIDOR CAÍDO”.

```bash
PS C:\Users\Alumno> [void]$incidencia.Insert(0, "SERVIDOR CAIDO")
PS C:\Users\Alumno> $incidencia
SERVIDOR CAIDO
Monitor parpadea
Ratón no va
```

El técnico resuelve la incidencia del “Ratón”. Búscala por su nombre y elimínala de la lista.

```bash
PS C:\Users\Alumno> $incidencia.Remove("Ratón no va")
```

Imprime la lista actual de tareas pendientes y cuenta cuántas quedan.

```bash
PS C:\Users\Alumno> $incidencia
SERVIDOR CAIDO
Monitor parpadea
PS C:\Users\Alumno> $incidencia.Count
2
```

4. Validación de lista negra de IPs (Seguridad)

Tu script de firewall recibe IPs sospechosas y debe decidir si bloquearlas o no.

Pasos a realizar:

Inicializa un ArrayList con las IPs ya bloqueadas: 10.10.10.5, 192.168.50.4, 80.80.80.80.

```bash
PS C:\Users\Alumno> $listanegra = New-Object System.Collections.ArrayList
PS C:\Users\Alumno> [void]$listanegra.Add("10.10.10.5")
PS C:\Users\Alumno> [void]$listanegra.Add("192.168.50.4")
PS C:\Users\Alumno> [void]$listanegra.Add("80.80.80.80")
PS C:\Users\Alumno> $listanegra
10.10.10.5
192.168.50.4
80.80.80.80
```

Declara una variable $nuevaAmenaza = "192.168.50.4".

```bash
PS C:\Users\Alumno> $nuevaAmenaza = "192.168.50.4"
```

Usa una estructura if para:

Si la IP ya está en la lista, muestra: “La IP [IP] ya estaba bloqueada. No se hace nada.”

Si no está, añádela a la lista y muestra: “IP [IP] añadida a la lista negra.”

Ya estaba en la lista, así que el resultado es:

```bash
PS C:\Users\Alumno> if ($listanegra.Contains($nuevaAmenaza)) {
>>     Write-Host "La IP [$nuevaAmenaza] ya estaba bloqueada. No se hace nada."
>> }
>> else {
>>     $listanegra.Add($nuevaAmenaza)
>>     Write-Host "IP [$nuevaAmenaza] añadida a la lista negra"
>> }
La IP [192.168.50.4] ya estaba bloqueada. No se hace nada.
PS C:\Users\Alumno>
```

Muestra la lista final ordenada alfabéticamente

```bash
PS C:\Users\Alumno> $listanegra | Sort-Object
10.10.10.5
192.168.50.4
80.80.80.80
```

Bloque III: Listas genéricas

1. Hardening de puertos de Firewall (List[int])

Vamos a configurar una regla de firewall que solo permite ciertos puertos numéricos.

Pasos a realizar:

Instancia una nueva lista genérica que solo acepte Enteros: [System.Collections.Generic.List[int]]::new().

```bash
PS C:\Users\Alumno> $puertos = New-Object System.Collections.Generic.List[int]
```

Añade los puertos estándar: 80, 443, 53.

```bash
PS C:\Users\Alumno> $puertos.Add(80)
PS C:\Users\Alumno> $puertos.Add(443)
PS C:\Users\Alumno> $puertos.Add(53)
PS C:\Users\Alumno> $puertos
80
443
53
PS C:\Users\Alumno>
```

Por error, alguien abrió el puerto Telnet (23). Añádelo a la lista.

```bash
PS C:\Users\Alumno> $puertos.Add(23)
PS C:\Users\Alumno> $puertos
80
443
53
23
PS C:\Users\Alumno>
```

Auditoría de seguridad: Elimina el puerto 23 de la lista.

```bash
PS C:\Users\Alumno> $puertos.Remove(23)
True
PS C:\Users\Alumno> $puertos
80
443
53
PS C:\Users\Alumno>
```

Intenta añadir (solo escribe la línea comentada) el texto “HTTP” a la lista y explica en un comentario qué error daría.

```bash
PS C:\Users\Alumno> $puertos.Add('HTTP')
No se puede convertir el argumento "item", con el valor: "HTTP", para "Add", al tipo "System.Int32": "No se puede
convertir el valor "HTTP" al tipo "System.Int32". Error: "La cadena de entrada no tiene el formato correcto.""
```

Muestra la lista de puertos permitidos ordenada de menor a mayor.

```bash
PS C:\Users\Alumno> $puertos | Sort-Object
53
80
443
```

Si fuera de mayor a menor:

```bash
PS C:\Users\Alumno> $puertos | Sort-Object -Descending
443
80
53
```

6. Inventario de servicios críticos (List[string])

Necesitas monitorizar una lista de servicios de Windows.

Pasos a realizar:

Crea una lista genérica de Strings a partir de un array existente (::new($array)): Array origen: Spooler, W3SVC, LanmanWorkstation.

```bash
PS C:\Users\Alumno> $arrayej6 = @("Spooler", "W3SVC", "LanmanWorkstation")
PS C:\Users\Alumno> $arrayej6
Spooler
W3SVC
LanmanWorkstation
PS C:\Users\Alumno>
```

Tuve que especificar que el array era de strings o daba error, también lo hice con AddRange:

```bash
PS C:\Users\Alumno> [string[]]$arrayString = $arrayej6
PS C:\Users\Alumno> $monitor = [System.Collections.Generic.List[string]]::new()
PS C:\Users\Alumno> $monitor.AddRange($arrayString)
PS C:\Users\Alumno> $monitor
Spooler
W3SVC
LanmanWorkstation
PS C:\Users\Alumno>
```

Te das cuenta de que falta el servicio de actualizaciones. Añade “wuauserv”.

```bash
PS C:\Users\Alumno> $monitor.Add("Wuauserv")
PS C:\Users\Alumno> $monitor
Spooler
W3SVC
LanmanWorkstation
Wuauserv
PS C:\Users\Alumno>
```

Ordena la lista alfabéticamente.

```bash
PS C:\Users\Alumno> $monitor | Sort-Object
LanmanWorkstation
Spooler
W3SVC
Wuauserv
PS C:\Users\Alumno>
```

Recorre la lista (foreach) y para cada elemento muestra el mensaje: “Monitorizando servicio: [NombreServicio]… OK”.

```bash
PS C:\Users\Alumno> $monitor | ForEach-Object {
>> Write-Host "Monitorizando servicio: $_... OK"
>> }
Monitorizando servicio: Spooler... OK
Monitorizando servicio: W3SVC... OK
Monitorizando servicio: LanmanWorkstation... OK
Monitorizando servicio: Wuauserv... OK
PS C:\Users\Alumno>
```

Bloque IV: Manipulación de texto

7. Análisis de Log de usuario

Tienes una línea de log cruda extraída de un servidor Linux. Necesitas sacar el usuario y la IP limpia.

Entrada: $logLine = " User: admin ; IP: 192.168.1.55 ; Status: Failed " (Nota los espacios extra al principio y final).

```bash
$logLine = " User: admin ; IP: 192.168.1.55 ; Status: Failed "
```

Pasos a realizar:

Limpia los espacios en blanco sobrantes del principio y el final de la cadena original.

```bash
PS C:\Users\Alumno> $logLine = $logLine.Trim()
PS C:\Users\Alumno> $logLine
User: admin ; IP: 192.168.1.55 ; Status: Failed
PS C:\Users\Alumno>
```

Divide la cadena usando el punto y coma ; como separador.

```bash
PS C:\Users\Alumno> $logLine = $logLine.Split(';')
PS C:\Users\Alumno> $logLine
User: admin
 IP: 192.168.1.55
 Status: Failed
PS C:\Users\Alumno>
```

El primer elemento será “User: admin”. Vuelve a dividirlo para quedarte solo con “admin”.

```bash
PS C:\Users\Alumno> $logLine[0].Substring(6)
admin
```

El segundo elemento es la IP. Límpialo para obtener solo “192.168.1.55”.

```bash
PS C:\Users\Alumno> $logLine[1].Substring(5)
192.168.1.55
```

Muestra un mensaje final claro: “ALERTA: El usuario [Usuario] intentó conectar desde [IP]”.

```bash
PS C:\Users\Alumno> Write-Host "ALERTA: El usuario [$($logLine[0].Substring(6).Trim())] intentó conectar desde [$($logLine[1].Substring(5).Trim())]."
ALERTA: El usuario [admin] intentó conectar desde [192.168.1.55].
```

(Puse el trim para eliminar espacios molestos dentro del propio array)

8. Generador de CSV para recursos humanos

RRHH te pide un listado de los correos de los nuevos empleados separados por comas para importarlos en su herramienta de Mailing.

Pasos a realizar:

Tienes tres variables con correos: $m1="ana@empresa.com", $m2="luis@empresa.com", $m3="bea@empresa.com".

```bash
PS C:\Users\Alumno> $m1="ana@empresa.com"
PS C:\Users\Alumno> $m2="luis@empresa.com"
PS C:\Users\Alumno> $m3="bea@empresa.com"
```

Crea un array temporal que contenga esas tres variables.

```bash
PS C:\Users\Alumno> $arrayej8 = @($m1, $m2, $m3)
PS C:\Users\Alumno> $arrayej8
ana@empresa.com
luis@empresa.com
bea@empresa.com
PS C:\Users\Alumno>
```

Utiliza el operador -join para crear una única cadena de texto separada por comas ,.

```bash
PS C:\Users\Alumno> $listacorreos = $arrayej8 -join ","
PS C:\Users\Alumno> $listacorreos
ana@empresa.com,luis@empresa.com,bea@empresa.com
PS C:\Users\Alumno>
```

Al resultado final, añádele manualmente el encabezado “EmailAddress,” al principio.

```bash
PS C:\Users\Alumno> $csv = "EmailAddress," + $listaCorreos
```

Muestra el resultado, que debería parecerse al contenido de un archivo .csv.

```bash
PS C:\Users\Alumno> $csv
EmailAddress,ana@empresa.com,luis@empresa.com,bea@empresa.com
```
